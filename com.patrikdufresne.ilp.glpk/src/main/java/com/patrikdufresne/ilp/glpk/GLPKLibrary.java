	...
	private static void loadLibrary(String name) {
		...
		if (prop != null) {
			if ("32".equals(prop) && IS_64) { //$NON-NLS-1$
				throw new UnsatisfiedLinkError(
						"Cannot load 64-bit GLPK libraries on 32-bit JVM"); //$NON-NLS-1$
			}
			if ("64".equals(prop) && !IS_64) { //$NON-NLS-1$
				throw new UnsatisfiedLinkError(
						"Cannot load 32-bit GLPK libraries on 64-bit JVM"); //$NON-NLS-1$
			}
		}
		...
		if (path != null) {
			if (extractAndLoad(libName1, path + SEPARATOR + fileName1, os()
				...
				return;
			if (extractAndLoad(libName2, path + SEPARATOR + fileName2, os()
					+ SEPARATOR + arch() + SEPARATOR + fileName2, message))
				return;
		}

		/* Failed to find the library */
		throw new UnsatisfiedLinkError(
				"Could not load GLPK library. Reasons: " + message.toString()); //$NON-NLS-1$
	}
